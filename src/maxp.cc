...
bool OpenTypeMAXP::Parse(const uint8_t *data, size_t length) {
  ...
  if (!table.ReadU32(&version)) {
    return Error("Failed to read version of maxp table");
  }

  if (version >> 16 > 1) {
    return Error("Bad maxp version %d", version);
  }

  if (!table.ReadU16(&this->num_glyphs)) {
    return Error("Failed to read number of glyphs from maxp table");
  }

  if (!this->num_glyphs) {
    return Error("Bad number of glyphs 0 in maxp table");
  }
  ...
  if (version >> 16 == 1) {
    ...
    if (!table.ReadU16(&this->max_points) ||
      ...
      return Error("Failed to read maxp table");
    }

    if (this->max_zones == 0) {
      // workaround for ipa*.ttf Japanese fonts.
      Warning("bad max_zones: %u", this->max_zones);
      this->max_zones = 1;
    ...
    if ((this->max_zones != 1) && (this->max_zones != 2)) {
      return Error("Bad max zones %d in maxp", this->max_zones);
    }
  } else {
...
bool OpenTypeMAXP::Serialize(OTSStream *out) {
  if (!out->WriteU32(this->version_1 ? 0x00010000 : 0x00005000) ||
    ...
    return Error("Failed to write maxp version or number of glyphs");
  }
